...
def init_db():
    with sqlite3.connect('cleanspace.db') as conn:
        ...
        # SAFET NET: Upgrade older databases to include passwords
        try:
            c.execute("ALTER TABLE staff ADD COLUMN password TEXT")
            c.execute("UPDATE staff SET password = staff_id || '123'") # Default password for old entries
        ...
        if c.fetchone()[0] == 0:
            ...
            for sid, name in initial_staff.items():
                ...
                unique_pass = f"{sid}123" # e.g. m_01123
...
# NEW: Staff Authentication Model
...
def get_all_staff():
    with sqlite3.connect('cleanspace.db') as conn:
        c = conn.cursor()
        # Modifed to also fetch the password so the manager can see it
        c.execute("""
...
# MODIFIED: Endpoint automatically generates a secure password
